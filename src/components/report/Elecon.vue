<template>
  <div id="middleBottomRight">
    <div class="bg-color-black" ref="eleconRef"></div>
  </div>
</template>
<script>
/*
...
export default {
  ...
  methods: {
    ...
    // 初始化图表的方法
    initChart() {
      this.chartInstance = this.$echarts.init(this.$refs.eleconRef, this.theme)
      for (var i = 0; i < 60; i++) {
        this.eleconData.push(this.randomData(3, 5))
      }
      const initOption = {
        backgroundColor:"",
        title: {
    ...
    // 更新图表配置项
    updateChart() {
      this.eleconData.shift()
      this.eleconData.push(this.randomData(6, 3))
      const dataOption = {
        series: [
          {
            data: this.eleconData,
    ...
    screenAdapter() {
      const titleFontSzie = (this.$refs.eleconRef.offsetWidth / 100) * 3.6
